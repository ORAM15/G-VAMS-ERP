...
// backlogStatusSummary gives the decision runtime an explicit, hard-to-miss open/completed split on top of
// the raw BACKLOG.md text already included below -- this is advisory context only. The actual enforcement
// against re-selecting completed work is the deterministic Decision Gate (agent-gatekeeper.js
// validateDecision()), which does not rely on the model reading or honoring this section.
function backlogStatusSummary() {
  const text = readText(".agent/BACKLOG.md");
  if (text === null) return { open: [], completed: [] };
  const items = gatekeeper.parseBacklogItems(text);
  return {
    open: items.filter((item) => item.status && !["done", "rejected"].includes(item.status)).map((item) => item.id),
    completed: items.filter((item) => item.status === "done").map((item) => {
      const evidenceMatch = text.slice(item.start, item.end).match(/^-\s*\*\*Completion evidence:\*\*\s*(.+)$/m);
      return { id: item.id, evidence: evidenceMatch ? evidenceMatch[1].trim() : null };
    }),
  };
}
...
function buildContext() {
  ...
  const backlogStatus = backlogStatusSummary();
  const context = {
    schema_version: 1,
...
function markdown(context) {
  ...
  lines.push("## Backlog status summary", "");
  lines.push("Eligible for selection (open/in-progress/blocked):");
  (context.backlogStatus.open.length ? context.backlogStatus.open : ["none"]).forEach((id) => lines.push(`- ${id}`));
  lines.push("");
  lines.push("Already completed -- do NOT re-select, the deterministic Decision Gate will reject it:");
  (context.backlogStatus.completed.length ? context.backlogStatus.completed : [{ id: "none", evidence: null }]).forEach((item) => lines.push(`- ${item.id}${item.evidence ? ` (${item.evidence})` : ""}`));
  lines.push("");
  lines.push("## Git", "");
