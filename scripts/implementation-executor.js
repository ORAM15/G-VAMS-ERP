...
/**
 * Resolves a provider name to its Provider Adapter function. Fails closed on an unknown provider -- v1
 * implements only stub-deterministic-v1.
 * @param {string} name
...
function resolveProvider(name) {
  ...
  if (!provider) {
    throw new Error(`Unknown execution provider "${name}". Implementation Executor v1 only implements: ${Object.keys(PROVIDERS).join(", ")}.`);
  }
  return provider;
}
 ...
 * schema (status/modifiedFiles/testsExecuted/testsPassed/warnings/errors/executionSummary). This is the one
 * place any future provider's response shape gets translated -- callers downstream of this function never
 * need to know which provider actually ran. Fails closed on an unrecognized outcome rather than guessing.
 * @param {object} raw a Provider Adapter's raw response
 * @returns {{status: string, modifiedFiles: string[], testsExecuted: number, testsPassed: number, warnings: string[], errors: string[], executionSummary: string}}
 */
...
function renderMarkdown(execution) {
  ...
  lines.push(
    "Generated by `scripts/implementation-executor.js` -- deterministic, no AI/LLM involved. v1 invokes only a deterministic stub Provider Adapter; it never generates recommendations, analyzes the repository, or makes engineering decisions.",
    ""
  );
...
/**
 * Writes execution.json and execution.md into the output directory (created if needed), returning their
 * absolute paths.
